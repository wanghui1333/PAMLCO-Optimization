...
zuiyou_jianju = data_results(5, :);
zuiyou_daoliu = data_results(6, :);
zuiyou_xzuobiao = zuiyou_xzuobiao - zuiyou_banchang;

% zuiyou_xzuobiao = zuiyou_xzuobiao(2);
% zuiyou_yzuobiao = zuiyou_yzuobiao(2);
% zuiyou_banchang = 120;
% zuiyou_jingchang = 1600;
% zuiyou_jianju(:)=60;
% zuiyou_daoliu(:)=7;
...
for idx=1:jingshu
    if flag(idx)==1
        ...
        plot([jing_zuobiao(idx, 1), pingtai_zuobiao(lianjie_flag(idx), 1)], [jing_zuobiao(idx, 2), pingtai_zuobiao(lianjie_flag(idx), 2)]);
        jinchi = get_jinchi_top([jing_zuobiao(idx, 1), jing_zuobiao(idx, 2)-zuiyou_jingchang_reshape(idx)], zuiyou_jingchang_reshape(idx), pingtai_zuobiao(lianjie_flag(idx), :));
